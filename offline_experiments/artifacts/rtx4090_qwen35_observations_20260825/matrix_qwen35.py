...
def build_cfg(*, model, dataset_key, track, mbs, ga, attn, gpu_count, gc, zero,
              out_dir):
    m = MODELS[model]
...
def plan():
    """sdpa 主轨：4090 上 Qwen3.5 的实际最优路径。

    背景（已实测 184 组）：
      - 4090 是 Ada sm_89，跑不了 FA3（只支持 Hopper sm_90）；H800 走 FA3，
        因此不触发 llamafactory 的 qwen3_5 patcher，能自由扫 MBS。
      - 4090 上用 FA2 会触发该 patcher，它从 position_ids 推出 cu_seqlens
        传给 fla，fla 要求此时 batch 必须为 1 —— 于是 MBS 只能是 1。
      - sdpa 不触发 patcher，MBS 可自由取值。同为 MBS=1 时 sdpa 与 FA2
        吞吐基本持平（22 组对比，多数 1.0-1.3 倍）；而 sdpa 能开大 MBS，
        实测最优比 FA2 靠梯度累积凑的最优快 1.7-2.8 倍。

    所以主力数据放在 sdpa + 扫 MBS 上；FA2 那 122 条成功数据保留作对照。
    """
    jobs = []
                    ...
                    if m["single_card_ok"]:
                        for gc in (False, True):
                            jobs.append(dict(model=model, track=track,
                                             dataset=ds_key, mbs=mbs, ga=1,
                                             attn="sdpa", gpu_count=1, gc=gc,
                                             zero="none"))
                    for gpu_count in (2, 4):
                        # ZeRO-3 只对装不下的量级有价值：9B 在 2 卡上 ZeRO-2
                        # 直接 OOM、ZeRO-3 才装得下；但 4 卡 ZeRO-2 装得下时
                        # ZeRO-3 是纯亏（实测 966 vs 127 tok/s）。
                        zeros = ("z2", "z3") if model == "qwen35_9b" else ("z2",)
                        for zero in zeros:
                            jobs.append(dict(model=model, track=track,
                                             dataset=ds_key, mbs=mbs, ga=1,
                                             attn="sdpa", gpu_count=gpu_count,
                                             gc=False, zero=zero))
    return jobs
...
def acquire_lock():
    ...
    并发跑两个矩阵会让显存采样互相污染，并且制造假 OOM（已踩过）。
    """
    if LOCK.exists():
        pid = -1
        try:
...
def main():
    ...
    tagged = []
    for j in jobs:
        tag = (f"{j['model']}_{j['track']}_{j['dataset']}_{j['attn']}"
               f"_mbs{j['mbs']}_ga{j['ga']}_g{j['gpu_count']}"
               f"_gc{int(j['gc'])}_{j['zero']}")
...
def run_all(tagged, done, args, results):
    for i, (tag, j) in enumerate(tagged, 1):
        ...
        cfg = build_cfg(model=j["model"], dataset_key=j["dataset"],
                        track=j["track"], mbs=j["mbs"], ga=j["ga"],
                        attn=j["attn"], gpu_count=j["gpu_count"],
                        gc=j["gc"], zero=j["zero"], out_dir=out_dir)
