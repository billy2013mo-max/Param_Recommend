...
Run:  python drive_h800_hybrid_vl_prospective_acceptance_v3.py [--dry-run]
"""
...
from prepare_h800_hybrid_vl_prospective_acceptance_v3 import COMBINED_QUEUE, PHASE_ID

PYTHON = Path("/fine-tuning-launcher/.venv/bin/python")
GPU_IDS = list(range(8))
STATUS = RUNTIME_DIR / "h800_hybrid_vl_prospective_acceptance_v3_driver_status.json"
...
def _status_snapshot(jobs: list[dict[str, Any]], running: dict[str, list[int]]) -> dict[str, Any]:
    ...
    return {
        "schema": "sft_h800_hybrid_vl_prospective_acceptance_v3_driver_status",
        "updated_at_utc": _now(),
        "phase_id": PHASE_ID,
        "total_jobs": len(jobs),
