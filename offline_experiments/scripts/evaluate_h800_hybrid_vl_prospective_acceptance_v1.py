...
OUTPUT = ARTIFACT_DIR / "h800_hybrid_vl_prospective_acceptance_report_v1.json"
...
def _observed(job: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "visual_semantics_passed": (
            ...
            if job.get("design_arm") == "vl_image_prospective" and classification == "success"
            else job.get("design_arm") != "vl_image_prospective" or classification == "oom"
        ),
    }
...
def _join_rows() -> list[dict[str, Any]]:
    ...
    for job in read_jsonl(COMBINED_QUEUE):
        ...
        track = "vl_image" if job.get("design_arm") == "vl_image_prospective" else "hybrid"
        if track == "vl_image":
