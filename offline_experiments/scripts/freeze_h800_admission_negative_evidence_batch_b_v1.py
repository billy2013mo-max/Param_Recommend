#!/usr/bin/env python3
"""Freeze and optionally promote the negative-evidence batch B approval.

Batch B is the 1- and 2-GPU subset (42 of the 72 frozen jobs), confined to
GPUs 0-3 at the user's instruction so 4-7 stay available to other people.  The
30 four-GPU jobs are deliberately out of scope and need their own approval.
...
AUTHORIZATION = (
    "用户在 2026-08-07 明确批准：修改 config/experiment.json 的 training_scope，"
    "并仅在 GPU 0-3 上运行显存准入负样本补数实验的 1 卡与 2 卡子集（42 个作业）。"
    "GPU 4-7 留给外部使用，不得占用。本 approval 只允许执行冻结的这 42 个作业，"
    "不得抢占外部进程、不得复用旧 campaign 的 approval、不得扩展到 30 个 4 卡作业，"
...
EXPECTED_JOB_COUNT = 42
AUTHORIZED_GPU_IDS = [0, 1, 2, 3]
MAX_GPU_COUNT = 2

DEFAULT_QUEUE = (
    ROOT / "matrix" / "h800_admission_negative_evidence_batch_b_jobs_v1.jsonl"
)
...
CANDIDATE = (
    ARTIFACT_DIR
    / "approval_design_h800_admission_negative_evidence_batch_b_v1_candidate.json"
)
...
SCOPED_SOURCE_RELATIVE_PATHS = (
    ...
    "scripts/freeze_h800_admission_negative_evidence_batch_b_v1.py",
    "scripts/run_job.py",
...
def _validate_queue(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        if gpu_count not in (1, 2):
            raise ValueError(
                f"batch B is the 1/2-GPU subset; found gpu_count={gpu_count}. "
                "Four-GPU jobs require a separate approval."
            )
