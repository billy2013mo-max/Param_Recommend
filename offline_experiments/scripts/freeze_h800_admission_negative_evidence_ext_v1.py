...
AUTHORIZATION = (
    "用户在 2026-08-08 选择方案 A：为 LoRA 1 卡/检查点开 补 3 个更长业务源的高压点，"
    "以取得该机制唯一缺失的失败样本。仅在 GPU 0-3 上运行这 3 个单卡作业。"
...
DEFAULT_QUEUE = (
    ROOT / "matrix" / "h800_admission_negative_evidence_ext_jobs_v1.jsonl"
)
DEFAULT_DESIGN = ARTIFACT_DIR / "h800_admission_negative_evidence_ext_design_v1.json"
DEFAULT_QUEUE_MANIFEST = (
    ARTIFACT_DIR / "h800_admission_negative_evidence_queue_manifest_v1.json"
)
CANDIDATE = (
    ARTIFACT_DIR
    / "approval_design_h800_admission_negative_evidence_ext_v1_candidate.json"
)
...
SCOPED_SOURCE_RELATIVE_PATHS = (
    ...
    "scripts/freeze_h800_admission_negative_evidence_ext_v1.py",
    "scripts/run_job.py",
...
def _validate_queue(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        if row.get("mechanism_id") != "lora_zero0_gc1_1gpu_pack0":
            raise ValueError(
                "this extension only covers LoRA 1-GPU / checkpointing-on, the one "
                f"mechanism still lacking a failure; found {row.get('mechanism_id')}"
            )
        if row.get("probe_role") != "probe_high":
            raise ValueError(
                "the extension is high-pressure only; the mechanism already has six "
                "safe observations, so another control point adds nothing"
            )
        if gpu_count != 1:
            raise ValueError(
...
def freeze() -> Path:
    ...
    if unknown:
        raise ValueError(f"extension queue holds job ids absent from its design: {unknown}")
