#!/usr/bin/env python3
"""Freeze and promote the exact 35-job final-blind resume subset."""
...
from prepare_h800_final_memory_business_blind_resume_v2 import (
    CAMPAIGN_ID, DEFAULT_DESIGN, DEFAULT_EXPERIMENT, DEFAULT_QUEUE,
    EXPECTED_RESUME, ORIGINAL_DESIGN, ORIGINAL_PREDICTIONS, ORIGINAL_QUEUE, PHASE_ID,
)
...
AUTHORIZATION = (
    "用户要求继续完成最终显存盲测。原60任务在完成20个后因一个未执行的2卡ZeRO-0场景被launcher拦截。"
    "本批准仅允许续跑原结果前冻结队列中尚未启动且执行器合法的35个任务；每行payload必须与原队列逐字节语义一致，"
    "仅使用GPU 0-3，不重跑20个完成任务，不运行5个无效ZeRO-0任务，不修改冻结预测或线上模型。"
)
DEFAULT_CANDIDATE = ARTIFACT_DIR / "approval_design_h800_final_memory_business_blind_resume_v2_candidate.json"
...
def main() -> None:
    ...
    original = {str(row["job_id"]): row for row in read_jsonl(ORIGINAL_QUEUE)}
    design = read_json(DEFAULT_DESIGN)
    if (
        len(rows) != EXPECTED_RESUME
        or len({str(row["job_id"]) for row in rows}) != EXPECTED_RESUME
        or any(original.get(str(row["job_id"])) != row for row in rows)
        or any(int(row["gpu_count"]) > 1 and int(row["zero_stage"]) not in {2, 3} for row in rows)
        or any((ROOT / "results" / str(row["job_id"])).exists() for row in rows)
        or design.get("resume_job_ids") != [str(row["job_id"]) for row in rows]
        or design.get("resume_ordered_payload_sha256") != sha256_json(rows)
        or design.get("bindings", {}).get("resume_queue", {}).get("sha256") != sha256_file(DEFAULT_QUEUE)
        or design.get("bindings", {}).get("original_queue", {}).get("sha256") != sha256_file(ORIGINAL_QUEUE)
        or design.get("bindings", {}).get("original_predictions", {}).get("sha256") != sha256_file(ORIGINAL_PREDICTIONS)
    ):
        raise ValueError("exact resume subset or frozen binding drifted")
    live = read_json(ROOT / "config" / "experiment.json")
    scope = live.get("training_scope") or {}
    if scope.get("phase_id") != PHASE_ID or scope.get("gpu_ids") != list(prep.GPU_IDS) or int(scope.get("max_gpu_count", 0)) != 4:
        raise ValueError("live config is not the exact resume scope")
    hardware = _hardware()
    ...
    launcher = ROOT / "final_memory_business_blind_resume_v2_staging" / "launch_h800_final_memory_business_blind_resume_v2.py"
    sources = [
        ...
        SCRIPTS / "prepare_h800_final_memory_business_blind_resume_v2.py",
    ...
    bound = {
        DEFAULT_QUEUE, DEFAULT_DESIGN, DEFAULT_EXPERIMENT, ORIGINAL_QUEUE, ORIGINAL_DESIGN,
        ORIGINAL_PREDICTIONS, prep.V3_ARTIFACT, prep.MEASUREMENT_GATE, prep.DATASET_INFO,
    ...
    candidate = {
        ...
        "campaign_design": {"path": str(DEFAULT_DESIGN.resolve().relative_to(ROOT.resolve())), "sha256": sha256_file(DEFAULT_DESIGN)},
        "file_sha256": manifest,
        "execution_order": [PHASE_ID],
        "allowed_job_ids": [str(row["job_id"]) for row in rows],
        "authorized_gpu_ids": list(prep.GPU_IDS),
        "max_gpu_count": 4,
        "scheduler_execution": {"join_busy_pool": False, "preemption_allowed": False, "policy": "exact pending-valid subset on GPU 0-3"},
    ...
    print(json.dumps({"candidate": str(DEFAULT_CANDIDATE), "sha256": digest, "jobs": EXPECTED_RESUME, "promotion": report}, ensure_ascii=False, indent=2))
