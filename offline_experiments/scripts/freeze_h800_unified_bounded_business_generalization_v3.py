#!/usr/bin/env python3
"""Freeze and optionally promote the 12-job real-business V3 validation."""
...
AUTHORIZATION = (
    "用户在2026-08-10明确要求不用三个历史验证数据集，而是在真实业务数据集上补充泛化实验并验收当前V3中心精度。"
    "本批准仅允许冻结V3后的12个业务泛化任务：四个未进入V3开发表的真实业务数据集各3个配置；"
    "使用GPU 0-7，每任务1或2卡，GBS32，1步warmup+4步measure，不packing、不offload、不重试。"
    "不得抢占其他用户进程，不得执行旧34任务，不得在看到结果后修改冻结预测，不得发布或覆盖线上模型。"
)
DEFAULT_CANDIDATE = ARTIFACT_DIR / "approval_design_h800_unified_bounded_business_generalization_v3_candidate.json"
...
def freeze(queue: Path, design_path: Path, predictions_path: Path, experiment_path: Path, candidate_path: Path) -> Path:
    ...
    if (
        ...
        raise ValueError("queue is not the exact 12-job real-business V3 validation")
    for row in rows:
        model = models[str(row["model_id"])]
        if (
            Path(str(row["model_path"])).resolve() != Path(str(model["path"])).resolve()
            or Path(str(row["tokenizer_path"])).resolve() != Path(str(model["path"])).resolve()
            or int(row["model_parameters"]) != int(model["actual_parameters"])
            or int(row["gradient_accumulation_steps"]) * int(row["gpu_count"]) * int(row["mbs"]) != TARGET_GBS
        ):
            raise ValueError(f"model or batch identity mismatch: {row['job_id']}")
        partition = row.get("calibration_partition") or {}
        if (
            partition.get("role") != "prospective_acceptance"
            or partition.get("policy") != "unified_bounded_v3_real_business_generalization_v1"
        ):
    ...
    if (
        ...
        or design.get("bindings", {}).get("experiment", {}).get("sha256") != sha256_file(experiment_path)
        or design.get("bindings", {}).get("frozen_predictions", {}).get("sha256") != sha256_file(predictions_path)
        or predictions.get("status") != "frozen_before_any_business_outcome"
        ...
        raise ValueError("business validation design or prediction binding drifted")
    ...
    if (
        ...
        raise ValueError("live experiment config is not the business-validation scope")
    ...
    scoped_source_paths = [
        ...
        SCRIPTS / "prepare_h800_unified_bounded_business_generalization_v3.py",
        SCRIPTS / "evaluate_h800_unified_bounded_business_generalization_v3.py",
        Path(__file__).resolve(),
        ROOT / "unified_bounded_business_generalization_staging" / "launch_h800_unified_bounded_business_generalization_v3.py",
    ...
    approval_design = {
        ...
        "frozen_predictions": {"path": str(predictions_path.resolve().relative_to(ROOT.resolve())), "sha256": sha256_file(predictions_path)},
        ...
        "max_gpu_count": 2,
        "scheduler_execution": {"join_busy_pool": False, "preemption_allowed": False, "policy": "parallel disjoint one/two-GPU masks across GPU 0-7"},
        "oom_policy": "no_retry; OOM is right-censored and fails this center-accuracy acceptance",
...
def main() -> None:
    ...
    candidate = freeze(args.queue.resolve(), args.design.resolve(), args.predictions.resolve(), args.experiment.resolve(), args.candidate.resolve())
