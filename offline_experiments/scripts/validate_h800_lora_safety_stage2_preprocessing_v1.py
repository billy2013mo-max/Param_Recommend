#!/usr/bin/env python3
"""Validate all 20 stage-two profiles against installed LLaMA-Factory preprocessing."""
...
DEFAULT_OUTPUT = ARTIFACT_DIR / "h800_lora_safety_stage2_preprocessing_validation_v1.json"
...
def processor_run(scenario: dict[str, Any]) -> Any:
    ...
    config = {
        ...
        "packing": False,
        "output_dir": str(RUNTIME_DIR / "h800_lora_safety_stage2_preprocessing_validation"),
...
def main() -> None:
    ...
    if args.output.exists():
        raise SystemExit(f"refusing to overwrite existing validation: {args.output}")
    datasets.disable_progress_bars()
    bundle = read_json(args.bundle)
    scenarios = list(bundle.get("scenarios") or [])
    if len(scenarios) != EXPECTED_SCENARIOS:
        raise ValueError(f"expected {EXPECTED_SCENARIOS} scenarios, got {len(scenarios)}")
    checks = []
    for index, scenario in enumerate(scenarios, start=1):
        print(
            f"[{index:02d}/{EXPECTED_SCENARIOS}] validating {scenario['dataset_id']} "
    ...
    report = {
        "schema": "sft_h800_lora_safety_stage2_preprocessing_validation/v1",
